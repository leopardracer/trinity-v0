...
const circuitParser = function (text) {
  ...
  while (i < text.length) {
    // console.log("gate", gatesCount); // DEBUGGING
    ...
    // console.log("operation", operation); // DEBUGGING

    // parse gate Id
    i++;
    gateId = "";
    while (text.charAt(i) !== "[") {
      // console.log("looking for [", i); // DEBUGGING
    ...
    while (
      ...
      // console.log("looking for op", i); // DEBUGGING
    ...
    circuit.gates.push(
      new Gate(gateId, operation, inputWires, outputWire, truthTable)
    );
    gatesCount--;
  }
