...
module.exports = function (text) {
  ...
  if (rows.length !== parseInt(rows[0][0]) + 3) {
    throw new Error(
      "Circuit has inconsistent number of lines compared to gates count"
    );
  }
  ...
  for (let r = 3; r < rows.length; r++) {
    ...
    if ((operation === "INV" || operation === "NOT") && inputs.length !== 1) {
      throw new Error(
        operation + " Gate " + r + " does not have exactly 1 input!"
      );
    }
    if (
      (operation === "AND" || operation === "LOR" || operation === "XOR") &&
      inputs.length !== 2
    ) {
