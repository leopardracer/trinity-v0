...
const evaluateAnd = function (agent, garbledGate, garbledAssignment) {
  ...
  garbledAssignment[out] = crypto.decrypt(
    label1,
    label2,
    garbledGate.id,
    cipher
    ...
    receiveInputLabels(agent, circuit).then(function (garbledAssignment) {
      ...
      for (let i = 0; i < circuit.gates.length; i++) {
        ...
        if (garbledGate.operation === "AND") {
          ...
          garbledGate.operation === "NOT" ||
          garbledGate.operation === "INV"
        ) {
      ...
      const garbledOutput = garbledAssignment.slice(
        circuit.wiresCount - circuit.outputSize
      );
      agent.socket.send(
        "output",
        garbledOutput.map(function (label) {
...
module.exports = {
  evaluateCircuit: function (agent, serializedCircuit, garbledAssignment) {
    const circuit = circuitParser(serializedCircuit);
