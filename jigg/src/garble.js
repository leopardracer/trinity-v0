...
const garbleAnd = function (agent, gate, R, garbledAssignment) {
  ...
  let values = [
    crypto.encrypt(
      garbledAssignment[in1][0],
      garbledAssignment[in2][0],
      gate.id,
      garbledAssignment[out][0]
    ),
    crypto.encrypt(
      garbledAssignment[in1][0],
      garbledAssignment[in2][1],
      gate.id,
      garbledAssignment[out][0]
    ),
    crypto.encrypt(
      garbledAssignment[in1][1],
      garbledAssignment[in2][0],
      gate.id,
      garbledAssignment[out][0]
    ),
    crypto.encrypt(
      ...
      garbledAssignment[in2][1],
      gate.id,
      garbledAssignment[out][1]
    ),
  ];

  let points = [
    2 * garbledAssignment[in1][0].getPoint() +
      garbledAssignment[in2][0].getPoint(),
    2 * garbledAssignment[in1][0].getPoint() +
      garbledAssignment[in2][1].getPoint(),
    2 * garbledAssignment[in1][1].getPoint() +
      garbledAssignment[in2][0].getPoint(),
    2 * garbledAssignment[in1][1].getPoint() +
      garbledAssignment[in2][1].getPoint(),
  ];
...
const degarbleOutput = function (agent, garbledAssignment, outputLabels) {
  ...
  for (let i = 0; i < agent.circuit.outputSize; i++) {
    ...
    const options =
      garbledAssignment[
        agent.circuit.wiresCount - agent.circuit.outputSize + i
      ];
    if (options[0].equals(bitLabel)) {
      ...
      agent.progress(
        ...
        null,
        null,
        "output label unequal to either labels"
      );
...
module.exports = {
  generateGarbling: (agent) => {
    const circuit = agent.circuit;
    const garbledCircuit = new Circuit(
      circuit.wiresCount,
      circuit.garblerInputSize,
      circuit.evaluatorInputSize,
      circuit.outputSize,
      agent.labelSize
    );
    ...
    const garbledAssignment = generateInputLabels(
      R,
      circuit.evaluatorInputSize + circuit.garblerInputSize,
      agent.labelSize
    );
    ...
    return {
      circuit: garbledCircuit.serialize(),
      garblerInputSize: circuit.garblerInputSize,
      evaluatorInputSize: circuit.evaluatorInputSize,
      inputLabels: garbledAssignment.slice(
        0,
        circuit.garblerInputSize + circuit.evaluatorInputSize
      ),
      outputLabels: garbledAssignment.slice(
        circuit.wiresCount - circuit.outputSize,
        circuit.wiresCount
      ),
      R: R,
    };
  },
  degarbleOutput: degarbleOutput,
};
